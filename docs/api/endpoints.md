...
`token` is returned in full on creation and on listing, because the invite screen
has to rebuild the link to share it again.
...
## What the schema has to change

Recorded as they were found while the front end was built. None of them breaks
anything yet, because no endpoint exists.

1. **`ledger_id` replaces `user_id`** on accounts, categories and transactions.
   Add `ledgers`, `ledger_memberships`, `ledger_invites`.
2. **Opening balance per account per month.** Drop
   `accounts.initial_balance_cents`.
3. **Invite tokens ≥ 32 bytes, stored as a digest,** never generated by the
   client.
4. **`monthly_totals` as a real aggregate,** with the optional category filter.
5. **`created_by_id` stamped server-side** from the authenticated membership.
6. **`unaccent(lower(name))` on categories, with the unique index on the same
   expression.**
7. **`recurring_series` plus a `detached` column,** and the job that materialises
   the window forward.
8. **A "who am I" answer** in `/me` or `/ledgers`, so the app can say "Você".
9. **`Idempotency-Key` honoured** on every creating POST.
